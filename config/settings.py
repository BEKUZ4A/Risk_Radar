...
if not DEBUG:
    if SECRET_KEY.startswith('django-insecure-') or len(SECRET_KEY) < 50:
        raise RuntimeError('Production SECRET_KEY must be a long, secret value.')
    if not ALLOWED_HOSTS:
        raise RuntimeError('Production ALLOWED_HOSTS must be configured.')
...
if DB_ENGINE == 'postgres':
    DATABASES = {
        'default': {
            ...
            'NAME': os.getenv('DB_NAME', 'risk_radar'),
...
SPECTACULAR_SETTINGS = {
    'TITLE': 'Central Risk Engine API',
    'DESCRIPTION': 'Risk scoring, catalog, Stripe employee checkout, email OTP auth.',
    'VERSION': '1.1.0',
}
...
DEFAULT_FROM_EMAIL = os.getenv('DEFAULT_FROM_EMAIL', EMAIL_HOST_USER or 'noreply@riskradar.local')

# Stripe (faqat .env dan — hardcoded emas)
STRIPE_SECRET_KEY = os.getenv('STRIPE_SECRET_KEY', '')
STRIPE_PUBLISHABLE_KEY = os.getenv('STRIPE_PUBLISHABLE_KEY', '')
STRIPE_WEBHOOK_SECRET = os.getenv('STRIPE_WEBHOOK_SECRET', '')
if not DEBUG and not STRIPE_SECRET_KEY:
    raise RuntimeError('Production STRIPE_SECRET_KEY must be configured.')
